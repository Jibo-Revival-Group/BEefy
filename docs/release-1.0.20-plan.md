...
- STT / turn-finalization cleanup is now also in `polish`: the cloud side already covers the low-signal screen, filler-only rejection, and the `twerk` / `can you twerk` boundary, while the remaining proof target is the robot-side mishearing path
- broader personality and presence continuation is in `polish`: the favorites family, presence-aware greetings, and seasonal holiday batches are already source-backed, and the remaining work is mostly live playback / launch-state cleanup for the few prompts that still need robot proof
- the Pegasus edge-case MIM batch is now imported too: `CC_Fallback`, `what is your sign`, `how many people do you know`, `what is the loop`, `what is personal report`, `what is your IQ`, `what is Be a Maker`, and `what is your groundhog's name` are source-backed in the live catalog; the next sports / awards support-who/will-win batch is now in place too, with the two Pegasus gaps for Tour de France and Wimbledon filled locally so the lane can still be proven end to end
- the next identity/memory follow-up batch is now source-backed too: the `do you remember the time` reply stays on the memory path, and the `who is this` report-skill prompt continues to land in the report template bucket with placeholders preserved
- the next RI_USR capability batch is now source-backed too: `can you program jibo` uses the imported Be a Maker / Jibo Commander explanation in Build B, so the capability lane keeps growing in small testable slices
- the next RI_USR gift batch is now source-backed too: the Mother's Day, Father's Day, and speaker-birthday gift suggestions are now imported so the advice lane keeps growing in small testable slices
